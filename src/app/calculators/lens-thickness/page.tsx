...
export default function LensThicknessCalculatorPage() {
  ...
  const calculateThickness = () => {
    ...
    // Calculate decentration (per eye)
    const decentration = Math.abs(fPD - pPD) / 2

    // Effective diameter includes decentration
    const effectiveDiameter = width + 2 + (decentration * 2) // +2 for edge safety

    // Simplified thickness estimation using sag formula
    // Sag = r - sqrt(r² - (d/2)²) where r = (n-1)/Power (in meters)
    // Simplified: Edge thickness ≈ (Power × diameter²) / (8 × (n-1) × 1000) for minus
    // Plus adds center thickness

    const radiusOfCurvature = (selectedMaterial.index - 1) / (absPower / 1000) // in mm
    const halfDiameter = effectiveDiameter / 2

    // Sag calculation (approximate)
    let sag = 0
    if (radiusOfCurvature > halfDiameter) {
      ...
      // For very high powers, use approximation
    ...
    if (isMinusLens) {
      ...
      edgeThickness = 1.5 // Minimum edge for safety
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={framePD}
                    onChange={(e) => setFramePD(e.target.value)}
                    placeholder="e.g., 70"
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSelectedMaterial(lensMaterials.find(m => m.name === e.target.value) || lensMaterials[0])}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                >
                  {lensMaterials.map((material) => (
                    <option key={material.name} value={material.name}>
                      {material.name} (n={material.index})
                    </option>
                  ))}
                </select>
            ...
            {result && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6">
                ...
                <p className="text-xs text-emerald-600 mt-3 italic">
                  * These are estimates. Actual thickness varies based on base curve, cylinder power, and lab equipment.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="mt-8 bg-white rounded-xl border p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  <tr className="border-b">
                    <td className="py-2 pr-4">CR-39</td>
                    <td className="text-center py-2 px-2">1.50</td>
                    <td className="text-center py-2 px-2">Thickest</td>
                    <td className="py-2 pl-2 text-gray-600">Low Rx, best optics</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 pr-4">Polycarbonate</td>
                    <td className="text-center py-2 px-2">1.59</td>
                    <td className="text-center py-2 px-2">Medium</td>
                    <td className="py-2 pl-2 text-gray-600">Kids, safety, rimless</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 pr-4">Trivex</td>
                    <td className="text-center py-2 px-2">1.53</td>
                    <td className="text-center py-2 px-2">Medium</td>
                    <td className="py-2 pl-2 text-gray-600">Best of both worlds</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 pr-4">Hi-Index 1.60</td>
                    <td className="text-center py-2 px-2">1.60</td>
                    <td className="text-center py-2 px-2">Thin</td>
                    <td className="py-2 pl-2 text-gray-600">Rx ±4.00 to ±6.00</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 pr-4">Hi-Index 1.67</td>
                    <td className="text-center py-2 px-2">1.67</td>
                    <td className="text-center py-2 px-2">Thinner</td>
                    <td className="py-2 pl-2 text-gray-600">Rx ±6.00 to ±9.00</td>
                  </tr>
                  <tr>
                    <td className="py-2 pr-4">Hi-Index 1.74</td>
                    <td className="text-center py-2 px-2">1.74</td>
                    <td className="text-center py-2 px-2">Thinnest</td>
                    <td className="py-2 pl-2 text-gray-600">Rx &gt; ±9.00</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="mt-8 bg-white rounded-xl border p-6">
            ...
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex items-start gap-2">
                ...
                <span><strong>Choose smaller frames</strong> - Eye size has the biggest impact on edge thickness</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">•</span>
                <span><strong>Match frame PD to patient PD</strong> - Reduces decentration and edge thickness</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">•</span>
                <span><strong>Select appropriate material</strong> - Higher index for higher prescriptions</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">•</span>
                <span><strong>Consider aspheric design</strong> - Flatter curves = thinner edges</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">•</span>
                <span><strong>Roll and polish edges</strong> - Makes lenses appear thinner</span>
              </li>
            </ul>
          </div>
